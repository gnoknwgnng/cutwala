...
export const Home: React.FC = () => {
  ...
  return (
    <div className="relative flex-1 flex flex-col h-full w-full bg-[#f3f4f6] dark:bg-zinc-950 overflow-hidden select-none">
      ...
      {/* 2. Single Selected Shop Slide-Up Card matching compact horizontal reference layout */}
      <AnimatePresence>
        {selectedShop && (() => {
          ...
          return (
            <motion.div
              ...
              className="absolute bottom-5 left-3 right-3 sm:left-6 sm:right-6 z-20 max-w-2xl mx-auto w-[calc(100%-1.5rem)]"
            >
              <div 
                onClick={() => handleOpenDetails(selectedShop.shop_id)}
                className="bg-white dark:bg-zinc-900 rounded-3xl p-3 shadow-2xl border border-gray-200/80 dark:border-zinc-800 flex flex-row items-center gap-2.5 sm:gap-3.5 cursor-pointer relative group overflow-hidden"
              >
                {/* 1. Left: Shop Image Thumbnail */}
                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden shrink-0 bg-gray-100 dark:bg-zinc-800 relative border border-gray-200/50 dark:border-zinc-800">
                  <img 
                ...
                {/* 2. Center: Shop Info */}
                <div className="flex-1 flex flex-col justify-between min-w-0 h-20 sm:h-24 py-0.5 gap-0.5">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-[10px] sm:text-xs text-gray-500 dark:text-zinc-400 font-medium truncate">
                      (128 Reviews)
                    </span>
                  </div>
                ...
                <div className="flex flex-col justify-between items-end shrink-0 h-20 sm:h-24 gap-1 min-w-[130px] sm:min-w-[180px]">
                  {/* Top Right Controls: Distance + Heart + Dismiss */}
                  <div className="flex items-center gap-1.5 justify-end w-full">
                    <span className="text-[10px] sm:text-xs font-extrabold text-gray-800 dark:text-zinc-200 mr-0.5">
                    ...
                    {chairList.map((chair, i) => {
                      ...
                      return (
                        <div
                          ...
                          className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                            isOccupied
                  ...
                  <button
                    ...
                    className="w-full py-1.5 sm:py-2 px-4 sm:px-6 bg-[#ff0055] hover:bg-[#e0004c] text-white font-extrabold text-[11px] sm:text-xs rounded-xl shadow-md shadow-rose-500/25 transition-all active:scale-95 cursor-pointer text-center whitespace-nowrap"
                  >
                    Book Now
                  </button>
                </div>
