      ...
      {!selectedShop && (
        <div className="absolute bottom-6 left-4 right-4 z-10 max-w-sm mx-auto text-center pointer-events-none">
          <div className="bg-black/80 dark:bg-zinc-900/90 backdrop-blur-md text-white text-xs font-semibold py-2.5 px-4 rounded-full shadow-2xl border border-white/10 inline-flex items-center gap-2">
            <Compass className="h-4 w-4 text-amber-400 animate-spin" style={{ animationDuration: '6s' }} />
        ...
        {selectedShop && (
          ...
          >
            <div 
              ...
              className="bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl rounded-3xl p-4 shadow-2xl border border-amber-500/30 flex gap-4 cursor-pointer relative group overflow-hidden"
            >
              {/* Close Button */}
              <button
                onClick={(e) => {
                ...
                <div className="flex items-center justify-between border-t border-gray-100 dark:border-zinc-800 pt-2 mt-2">
                  <div className="flex items-center gap-2.5">
                    <span className="text-xs font-bold text-gray-800 dark:text-zinc-200 flex items-center gap-1">
                      <Star className="h-3.5 w-3.5 text-amber-500 fill-amber-500" /> {selectedShop.rating}
                    </span>
                    ...
                    <span className="text-[10px] font-bold text-gray-400 dark:text-zinc-500 flex items-center gap-0.5">
                      <MapPin className="h-3 w-3 text-amber-500" /> {getDistanceStr(selectedShop.shop_id)}
                    </span>
                  </div>
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 py-1.5 px-3 bg-amber-500 hover:bg-amber-600 text-black font-bold text-xs rounded-xl shadow-md transition-all active:scale-95 cursor-pointer shrink-0"
                  >
                    <span>Book</span>
